# 7.Turbulence-gated risk-off rule — non-learned safety valve (improvement #7 on top of #1–#6)

Variant of the base pipeline (`../0.original/`) that layers improvement #7 on top of the six prior improvements:
- **#1–#6** carried over from the previous folders.
- **#7** (new here): when the FinRL turbulence index exceeds a configured threshold on bar `t`, the agent's softmaxed weights are **overridden to zero** (= go to cash) BEFORE the TC penalty and reward shaping run. The portfolio earns 0% on that day; the TC penalty charges turnover on the forced liquidation / re-entry. The gate is the same in training and inference — the agent learns to coexist with it.

### Why it matters

The agent's policy is shaped by training-period data. During regime shifts that look nothing like training (COVID March 2020, 2008-Q4, August 2015 flash crash), the policy can produce confidently-wrong allocations that compound losses. A non-learned safety valve overrides the policy with a hard-coded "cash is safer than this" rule whenever a market-wide stress indicator spikes.

Turbulence (Kritzman & Li, 2010) is the Mahalanobis distance of the current cross-section of returns from a long-run mean. Normal markets sit at ~30–60. Recent crisis spikes:
- August 2011 US debt downgrade: ~120
- August 2015 flash crash: ~85
- December 2018 selloff: ~95
- March 2020 COVID: ~200+
- February 2022 inflation shock: ~110

A threshold around 70 typically cuts roughly the worst 1–3% of trading days. The cost is missing some sharp recoveries (March 23 2020 was +9% on the Dow); the benefit is avoiding the preceding drawdown.

### Why it matters

### How it works

1. **Stage 1 attaches the turbulence column.** With `data.use_turbulence = true`, FinRL's `FeatureEngineer.preprocess_data` computes the Kritzman & Li turbulence index from the trailing return covariance of all tickers and adds a `turbulence` column to every row of the dataset.
2. **Env reads it per bar.** `LogReturnPortfolioEnv._risk_off_active()` checks `self.data["turbulence"].values[0]` after each step. If it exceeds `risk_off.turbulence_threshold` AND `risk_off.enabled = true`, the gate is "on" for this bar.
3. **Gate runs first in `step()`.** When active, the env overwrites `actions_memory[-1]` with zeros, sets `portfolio_return_memory[-1] = 0`, and resets `portfolio_value` to the prior day's value. Cash holds.
4. **TC penalty applies on the forced rebalance.** With weights zeroed, `turnover = |0 - weights_{t-1}|.sum()` ≈ 1, so the env charges `transaction_cost_penalty * 1` to liquidate to cash. Going back into the market the next day charges another full turnover. Realistic.
5. **Reward shaping uses the (post-gate, post-TC) return.** `log(1 + r) * scaling`, DSR contribution, or DDR contribution — all computed on the effective return, which is zero (or slightly negative due to TC) on gated days.
6. **Stages 3, 4, 5 require no code changes.** Stage 3 invokes the env normally; the gate fires inside step() exactly as during training, so the stitched out-of-sample equity curve already reflects gated behaviour. Stages 4 and 5 just read the resulting weights CSV.

Setting `risk_off.enabled = false` short-circuits the gate entirely → byte-compatible fallback to folder 6.
...
Prerequisite: `data.use_turbulence` must be `true` (default in this folder). If it's false, stage 1 won't emit the `turbulence` column and the env's `_risk_off_active()` will silently return `False` for every bar (gate effectively off, with a one-time warning printed when the env is built).
...
### A.18 What was deliberately *not* changed
