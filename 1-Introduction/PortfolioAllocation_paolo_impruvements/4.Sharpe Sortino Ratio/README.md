# 4.Sharpe Sortino Ratio — differential Sharpe / Sortino reward (improvement #4 on top of #1 + #2 + #3)

Variant of the base pipeline (`../0.original/`) that layers improvement #4 on top of #1, #2, and #3:
...
- **#4** (new here): replace `log(1 + return)` with the **Moody & Saffell differential Sharpe ratio** (or a Sortino variant) so the agent trains on a *running risk-adjusted* signal rather than raw return. The agent now directly cares about variance (Sharpe) or downside variance (Sortino), not just expected return.

Math applied each non-terminal step (after optional TC adjustment to `r_t`):
```
A_t = A_{t-1} + eta * (r_t - A_{t-1})           EMA of return
B_t = B_{t-1} + eta * (r_t^2 - B_{t-1})         EMA of squared return
D_t = D_{t-1} + eta * (min(r_t,0)^2 - D_{t-1})  EMA of squared downside

DSR = (B_{t-1} * dA  -  0.5 * A_{t-1} * dB)  /  (B_{t-1} - A_{t-1}^2)^{1.5}
DDR = (D_{t-1} * dA  -  0.5 * A_{t-1} * dD)  /  D_{t-1}^{1.5}
```
where `eta ~ 1/252` corresponds to a one-trading-year EMA half-life. DSR is the exact derivative of `A/sqrt(B - A^2)` with respect to the new observation; DDR is the analogous derivative of `A/sqrt(D)`.

Activated by `env.reward_mode` in `config.json`:
- `"diff_sharpe"`  (default in this folder)
- `"diff_sortino"`
- `"log_return"`   → falls back to `../3. Walk-forward/` behaviour
- `"value"`        → falls back to `../0.original/` behaviour

Setting `transaction_cost_penalty = 0.0` and `reward_mode = "log_return"` reproduces `../1.reward_returns/` byte-for-byte. Walk-forward training/eval is independent of reward mode and stays on by default (see `walk_forward.enabled`).
...
### A.15 What was deliberately *not* changed
