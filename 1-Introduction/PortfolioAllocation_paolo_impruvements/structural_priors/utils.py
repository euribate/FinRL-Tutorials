...
def compute_prior_weights_from_cov(cov: np.ndarray,
    ...
    has_cash = cash_idx >= 0
    if has_cash:
        cs = cash_share if cash_share is not None else 1.0 / n
    ...
    if has_cash:
        mask = np.ones(n, dtype=bool); mask[cash_idx] = False
    else:
        mask = np.ones(n, dtype=bool)
    cov_risky = cov[np.ix_(mask, mask)] if has_cash else cov
    n_risky = int(mask.sum())

    if prior_type == "equal_weight" or n_risky == 0:
        risky_w = np.full(max(n_risky, 1), risky_total / max(n_risky, 1))
    elif prior_type == "inverse_vol":
        diag = np.diag(cov_risky).astype(float)
        vols = np.sqrt(np.maximum(diag, 1e-12))
        inv  = 1.0 / vols
        risky_w = (inv / inv.sum()) * risky_total
    elif prior_type == "risk_parity":
        rp = _solve_risk_parity(cov_risky)
        risky_w = rp * risky_total
    else:
        risky_w = np.full(max(n_risky, 1), risky_total / max(n_risky, 1))
