# 2.transaction_cost — TC-aware log-return reward (improvement #2 on top of #1)

Variant of the base pipeline (see `../0.original/`) that layers improvement #2 on top of improvement #1:
- **Improvement #1** (carried over from `../1.reward_returns/`): reward = `log(1 + portfolio_return) * reward_scaling` instead of raw portfolio value.
- **Improvement #2** (new in this folder): subtract a per-unit-turnover transaction-cost penalty from both the reward signal AND the book equity, so the agent learns to avoid churn and the stage 3/4/5 equity curves reflect realistic post-cost performance.

Math applied each non-terminal step (when `transaction_cost_penalty > 0`):
```
turnover     = |weights_t - weights_{t-1}|.sum()
tc_fraction  = transaction_cost_penalty * turnover
net_return   = (1 + gross_return) * (1 - tc_fraction) - 1
reward       = log(1 + net_return) * reward_scaling
```
The env also rewrites the just-appended entries of `portfolio_return_memory`, `asset_memory`, and resets `self.portfolio_value` so downstream consumers see net values.

Activated by `env.transaction_cost_penalty > 0` in `config.json` (default `0.001` in this folder). **Setting it to `0.0` short-circuits the TC math entirely — the env then behaves identically to `../1.reward_returns/`, byte-for-byte.** Setting `env.reward_mode` back to `"value"` restores `../0.original/`.
