...
def predict_walk_forward(algo: str, full_df: pd.DataFrame, config: dict,
    ...
    For each window, load that window's trained model and run it through a
    fresh env over the eval slice. Concatenate the per-window outputs in
    chronological order.
    """
    ...
    for i, (_ts, _te, es, ee) in enumerate(windows):
        ...
        ret_piece, act_piece = predict_one(
            algo, eval_slice, config, model_dir,
            model_filename=f"agent_{algo}_w{i}.zip",
        )
        return_pieces.append(ret_piece)
        action_pieces.append(act_piece)
    ...
    # df_actions from the env has the date as its index, not a column.
...
def main() -> None:
    ...
    if walk_forward:
        ...
        for algo in enabled_models(config):
            print(f"\nBacktesting {algo.upper()} (walk-forward, {len(windows)} windows)...")
            stitched_returns, stitched_actions = predict_walk_forward(
                algo, full_df, config, model_dir, windows
            )
            equity[algo.upper()] = daily_return_to_equity(stitched_returns, initial)
            # Persist the stitched weights so stage 4 can replay them.
        ...
        for algo in enabled_models(config):
            ...
            daily_return, df_actions = predict_one(algo, eval_df, config, model_dir)
            equity[algo.upper()] = daily_return_to_equity(daily_return, initial)
            weights_path = results_dir / f"weights_{algo}.csv"
            df_actions.to_csv(weights_path)
            print(f"Saved {weights_path}")
